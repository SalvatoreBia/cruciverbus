...
std::vector<DictEntry> get_input_dictionary(const std::string &filename)
{
    ...
    while (std::getline(file, str))
    {
        ...
        if (clue.size() >= 2 && clue.front() == '"' && clue.back() == '"') clue = clue.substr(1, clue.size() - 2);
        ...
        if (word_count < MIN_CLUE_WORDS) continue;
        
        dict.push_back({word, clue});
    }
    std::println(std::cout, "Dizionario caricato. Parole valide dopo il filtro: {}", dict.size());
...
typedef std::unordered_map<std::pair<size_t, size_t>, std::array<Slot, 2>> intersection_map;

std::pair<size_t, size_t> get_common_element(const std::vector<Cell> &a, const std::vector<Cell> &b)
{
...
intersection_map get_intersections(const std::vector<Slot> &slots)
{
    ...
    for (size_t i = 0; i < slots.size(); ++i)
    {
        for (size_t j = 0; j < slots.size(); ++j)
        {
            ...
            std::pair<size_t, size_t> intersection = get_common_element(slots[i].cells, slots[j].cells);
...
public:
    void initialize(const std::vector<Slot> &slots, const std::vector<DictEntry> &dict)
    {
...
public:
    ...
    void build_model(const std::vector<Slot> &slots, const DomainManager &domain_mgr)
    {
    ...
    void export_json(const std::string &filename, const grid &g, const std::vector<Slot> &slots) {
        ...
        auto write_dir = [&](bool hz, const char* key) {
            ...
            for (const auto &wv : word_vars) {
                if (kissat_value(solver, wv.sat_var) <= 0) continue;
